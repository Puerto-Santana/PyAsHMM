...
class AR_MOG_HMM:    
    def __init__(self,O,lengths,N,nC=None,C=None,P=None,A=None,pi=None,B=None,sigma=None,p_cor=5,diag=False):
        """
        Creates an object AR_MOG_HMM
        Based on the paper:
            B. H. Juang, ”Maximum likelihood estimation for mixturemultivariate 
            stochastic observations of Markov chains,” AT&TTech. I., vol. 64, 
            ...
            for i in range(N):
                ...
                for j in range(self.nC):
                    ...
                    for k in range(self.K):
                        ...
                        Bivk = np.concatenate([[(i+1)*(np.max(O.T[k])-np.min(O.T[k]))/(self.N+1)+np.min(O.T[k])],np.zeros(self.P)])
                        Biv.append(Bivk)
                    Biv = np.array(Biv)
                    Bi.append(Biv)
                B.append(Bi)
        ...
        for i in range(N):
            ...
            detsigmai = []
            for j in range(self.nC):
                sigmai.append(np.linalg.inv(self.sigma[i][j]))
                detsigmai.append(np.linalg.det(self.sigma[i][j]))
            self.invsigma.append(sigmai)
            self.detsigma.append(detsigmai)
    ...
    def pen(self,lengths):
        ...
        """
        T =np.sum(lengths)
        b = self.N*self.nC*(self.K*(1+self.P)+self.K*(self.K+1)/2.)+self.N**2+self.N
...
#  States management
    ...
    def mvn_param(self):
        ...
        #         for k in range(self.K):
        #             # muij[k] = self.C[i][j]*self.B[i][j][k][0]/(1-np.sum(self.B[i][j][k][1:]))
        #             muij[k] = self.B[i][j][k][0]/(1.-np.sum(self.B[i][j][k][1:]))
        #         mui.append(muij)
        #         # mui.append(np.sum(self.C[i][j]*muij))
        #     mu.append(mui)
...
#   HMM
    ...
    def viterbi(self,O,plot=True,maxg=False,absg=False,indexes=False,xlabel="Time units",ylabel="Values"): #esto esta bien?
        """
        ...
        if plot==True:
            plt.figure("Sequence of labeled hidden states of N-AR-MoG-HMM")
            plt.title("Sequence of labeled hidden state of N-AR-MoG-HMM")
            plt.ylabel(ylabel)
            plt.xlabel(xlabel)
            plt.plot(Q[::-1])
            plt.grid("on")
    ...
    def act_B(self):
        ...
        """
        aux_len = np.concatenate([[0],self.lengths.cumsum()])
        self.forBack[0].act_coef(self.O[aux_len[0]:aux_len[1]],self.P,self.N,self.sigma,self.invsigma)
        ac = self.forBack[0].coefvec
        bc = self.forBack[0].coefmat    
        for i in range(1,len(self.lengths)):
            self.forBack[i].act_coef(self.O[aux_len[i]:aux_len[i+1]],self.P,self.N,self.sigma,self.invsigma)
        ...
        for i in range(self.N):
            ...
            for j in range(self.nC):
                if np.prod(bc[i][j].shape)> 1:
                    Bij = []
                    BijT = np.linalg.solve(bc[i][j], ac[i][j])
                    for k in range(self.K):
                        Bij.append(BijT[k*(1+self.P):(k+1)*(1+self.P)])    
                else:
                    Bij = (ac[i][j]/bc[i][j])[0]
    ...
    def act_S(self):
        ...
        for i in range(self.N):
            for j in range(self.nC):
                for k in range(self.K):
                    covijk = cov[i][j][k][k]
                    if np.abs(covijk) < _eps:
                        cov[i][j][k][k] = np.sign(cov[i][j][k][k])*_eps
        return cov
    ...
    def save(self,root=None, name = r"hmm_"+str(datetime.datetime.now())[:10]):
        ...
        with open(root+"\\"+name+".armoghmm", 'wb') as fp:
            pickle.dump(itemlist, fp)
    
    def load(self,rootr):
        """
        Loads a model, must be an armoghmm file 
        ...
        """
        if rootr[-9:] != ".armoghmm":
            return "The file is not a armoghmm file"
...
class forback:
    ...
    def prob_t(self,O,B,N,P,K,nC,C,sigma,isigma,detsigma):
        ...
        Parameters: see AR_MOG_HMM class for more information
    ...
    def forward_backward(self,A,sigma,pi,O,P,C): 
        ...
        Parameters check AR_MOG-HMM for more information 
        """
        T = len(O)
    ...
    def forward_step(self,alfa,A,t):
        ...
        Parameters check AR_MOG_HMM for more information 
        ----------
    ...
    def act_coef(self,O,P,N,sigma,invsigma): #TODO: este falta modificarlo
        """
        ...
        for i in range(N):
            ...
            for j in range(nC):
                wij = w[i][j]
                numij = np.zeros([(1+P)*K])
                denij = np.zeros([(1+P)*K,(1+P)*K])
                for k in range(K):
                    for r in range(1+P):
                        if r == 0:
                            numij[k*(P+1)+r] = np.sum(wij*np.sum(O[P:]*invsigma[i][j][k],axis=1))
                        else:
                            numij[k*(P+1)+r] = np.sum(wij*np.sum(O[P:]*invsigma[i][j][k],axis=1)*O.T[k][P-r:-r])
                        for m in range(K):
                            for l in range(P+1):
                                if r==0 and l==0:
                                    denij[k*(P+1)+r][m*(P+1)+l] = invsigma[i][j][k][m]*np.sum(wij)
                                if r==0 and l!=0:
                                    denij[k*(P+1)+r][m*(P+1)+l] = invsigma[i][j][k][m]*np.sum(wij*O.T[m][P-l:-l])
                                if r!= 0 and l==0:
                                    denij[k*(P+1)+r][m*(P+1)+l] = invsigma[i][j][k][m]*np.sum(wij*O.T[k][P-r:-r])
                                if r!= 0 and l!=0:
                                    denij[k*(P+1)+r][m*(P+1)+l] = invsigma[i][j][k][m]*np.sum(wij*O.T[k][P-r:-r]*O.T[m][P-l:-l])
                numi.append(numij)
                deni.append(denij)
            ac.append(numi)
            bc.append(deni)
