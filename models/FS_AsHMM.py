# -*- coding: utf-8 -*-
"""
Created on Thu Aug 13 08:46:53 2020
...
class FS_AR_ASLG_HMM:    
    def __init__(self,O,lengths,N,rho= None ,p=None,P=None,A=None, pi=None,G=None,L=None,
                 ...
                 act_inde = True,rho_bar=0.9,true_rel=np.array([])):
        """
        ...
        true_rel : TYPE, optional numpy boolean array of size K
            DESCRIPTION. indicates which variables must be always relevant 
        ...
        if rho is None:
            self.rho = 0.1*np.ones(self.K) # prior de relevancias 
            if len(true_rel) > 0:
                t_ind = np.argwhere(true_rel ==1).T[0]
                self.rho[t_ind] = 1-0.00001
        else:
            self.rho = rho
        if true_rel is None:
            self.true_rel = np.zeros(self.K)
        else:
            self.true_rel = true_rel
...
#                    D.add_edges_from([(str(label[i])+"_AR_"+str(r+1),str(label[i]))])
    ...
    def climb_ar(self,tb,ts):
        ...
        """
        for i in range(self.N):
            accept = np.argwhere(self.rho>self.rho_bar)[:,0]
            for k in accept:
    ...
    def climb_struc(self,tb,ts): 
        ...
        """
        for i in range(self.N):
            accept = np.argwhere(self.rho>self.rho_bar).T[0]
...
#   HMM
    ...
    def act_rho(self):
        ...
        # rho2[ind] = 1-np.exp(-740)
        rho2[ind] = 1-0.00001
        t_ind = np.argwhere(self.true_rel==1).T[0]
        rho2[t_ind] = 1-0.00001
        return rho2
    ...
    def EM(self,its=500,err=1e-2,ta="all",tpi=True, tb="all",ts="all",ps=None): 
        """
        ...
        while eps >err and it<its:
            ...
            if eps<0:
                ...
                # print(str(it)+": "+str(np.sum(self.rho,axis=1))+", err:"+ str(eps))
                break
            self.prho = self.rho
            ...
            self.pA = np.copy(self.A)
            self.A = self.act_A(ta)
            self.ppi = np.copy(self.pi)
            if tpi == True:
                self.pi = self.act_pi()
            self.pB = np.copy(self.B)
            self.B = self.act_B2(self.G,self.p,tb)
            self.act_mut(self.G,self.B,self.p)
            self.psigma = np.copy(self.sigma)
    ...
    def SEM(self,err1=1 , err2=1e-2 , its1=10 , its2=500,ta="all",tpi=True, tb="all",ts="all",ps=None): 
        """
...
# Forecasting
    ...
    def save(self,root=None, name = "hmm_"+str(datetime.datetime.now())[:10]):
        ...
        itemlist = [self.N,self.K,self.P,self.rho,self.A,self.pi,self.B,self.sigma,
                    ...
                    self.rho_bar,self.true_rel]
        if self.kappa is not None:
            itemlist.append(self.nu)
            itemlist.append(self.kappa)
        with open(root+"\\"+name+".fsashmm", 'wb') as fp:
            pickle.dump(itemlist, fp)
    
    def load(self,rootr):
        """
        Loads a model, must be an fsashmm file 
        ...
        """
        if rootr[-8:] != ".fsashmm":
            return "The file is not an fsashmm file"
        ...
        self.true_rel   = loaded[20]
        if len(loaded)>21:
            self.nu     = loaded[21]
            self.kappa  = loaded[22]
...
class forback:
    ...
    def prob_BN(self,o,mu,sigma,epsi,tau,rho):  #Este si toca cambiar 
        ...
        temp_g = np.tile(g,[len(f),1])
        
        w_f  = (np.log(rho)+f)
        w_g  = (np.log(1-rho)+ temp_g)
